...
from eos.system_conformal_plasma import ConformalPlasma

from eos.variable_conversions import rho
from eos.variable_conversions import HBARC
from eos.variable_conversions import milne_T
from eos.variable_conversions import milne_mu
from eos.variable_conversions import milne_entropy
...
def denergy_dtau(
    ...
    return_value = derivative * denergy_drho(ys, rho(tau, r, q), **consts) * tau
    return_value -= 4.0 * \
        energy(temperature=temperature, chem_potential=chem_potential,**consts)
...
def denergy_dr(
    ...
    return derivative * denergy_drho(ys, rho(tau, r, q), **consts) / tau ** 4
...
def do_freezeout_surfaces(
    ...
    for k, alpha in enumerate(mu0_T0_ratios):
        ...
        e_interp = interp1d(rhos, [energy(*x, **consts) for x in
