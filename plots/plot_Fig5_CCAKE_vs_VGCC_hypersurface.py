...
from utils.analytic_functions import rho
...
def solve_equations(
    initial_conditions: np.ndarray, eom_instance: Any
...
def read_sim(simulation_path: str) -> pd.DataFrame:
    ...
    df["divE_r"] = (df["x"] * df["divE_x"] + df["y"] * df["divE_y"]) / df["r"]
    logger.info(f"Simulation data loaded from {simulation_file}.")
    return df


def find_freezeout_tau(
    e_interp: Callable, e_freezeout: float, r: float, q: float
) -> float:
    """
    ...
    try:
        tau = newton(
            lambda tau: e_freezeout - e_interp(rho(tau, r, q)) / tau**4, x0=0.1, x1=0.2
        )
        return tau
...
def solve_and_plot(
    ...
    for i, (x_val, tau_FO) in enumerate(freezeout_times[::skip_size]):
        ax.arrow(
            x=x_val, y=tau_FO, dx=normals[i, 0], dy=normals[i, 1], **ARROW_STYLE_AN
        )
    ...
    ax.scatter(sim_data["r"], sim_data["t"], **SIM_STYLE, label="CCAKE", zorder=10)
    ax.legend(
        frameon=False, fontsize=14, markerscale=2.0, handlelength=1.5, loc="lower left"
    )
    ax.text(
        0.13,
        0.3,
        r"$\hat{\mu}_{Y,0}/\hat{T}_0=$" + f" {mu_ratio:.2f}",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--simulation_path", required=True, help="Folder containing the CCAKE results."
    )
    parser.add_argument(
        "--output_path",
        required=True,
...
def main() -> None:
    ...
    fig, ax = plt.subplots(figsize=(3.36, 3.36), dpi=1200, constrained_layout=True)
