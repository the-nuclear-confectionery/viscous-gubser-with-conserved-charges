...
def plot(cfg: Config,
    ...
    """
    logger.info(f"Plotting the evolution of the system.")
    ...
    for n, tau in enumerate(taus):
        ...
        chemical_potential = milne_mu(
            ...
            r=xs,
            q=cfg.q,
            interpolated_mu_hat=mu_hat_interp[0]
        )
        energy = milne_energy(
            tau=tau,
            x=xs,
        ...
        number_density = ((4/3) * energy - temperature * entropy_density)/chemical_potential
...
def main() -> None:
    ...
    # Setup initial conditions.
    initial_conditions = np.array([cfg.T_hat_0, 7 * cfg.T_hat_0, 0.0, 0.0, cfg.pi_bar_hat_0])
    # Solve the equations of motion.
    solutions = solve_equations(initial_conditions, eom_instance)
    plot(cfg,
         ax1,
    ...
    # Third set (high μ)
    # Setup initial conditions.
    initial_conditions = np.array([cfg.T_hat_0, 9 * cfg.T_hat_0, 0.0, 0.0, cfg.pi_bar_hat_0])
    # Solve the equations of motion.
    solutions = solve_equations(initial_conditions, eom_instance)
    plot(cfg,
         ax1,
